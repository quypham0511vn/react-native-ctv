...
function openCamera(onSelected: any, _useFrontCamera?: boolean) {
    CheckPermission.hasCameraCaptureAndSave(async () => {
        await ImagePicker.openCamera({
            ...
            width: 500,
            height: 500,
            useFrontCamera: _useFrontCamera || false
        }).then((image) => {
        ...
        if (permission.camera !== 'authorized') {
            displayPermissionAlert(1, permission.camera, () => openCamera(onSelected, _useFrontCamera));
        }
    });
};
